/*---------------------------------------------------------------------------*/
/* Name : bxr_checker.c                                                      */
/* Info : Blue X-ray Server Checker                                          */
/* Version                                                                   */
/*  0.5 - New Create                                                         */
/*---------------------------------------------------------------------------*/
#include "bxrcommon.h"
...
void Proc_Stop(int signum)
{
	if (signum == SIGUSR1)
	{
		B_Log( INF, "Blue X-ray Checker Stop ...[%s %s]\n",__DATE__, __TIME__ );
...
void main()
{
	...
	B_Log( INF, "Blue X-ray Checker Start...[%s %s]\n",__DATE__, __TIME__ );
    ...
    for( idx = osName ; idx <= CheckListCount ; idx++) 
	{	
//		B_Log( DBG, "Check Complet... [%d]:[%d]\n", idx, CheckListCount );
		if( idx == CheckListCount )
		{
			B_Log( DBG, "Check Wait...  %d Sec... \n", WAIT_TIME );
			sleep(WAIT_TIME);
			idx -= (CheckListCount+1);
			continue;
		}
